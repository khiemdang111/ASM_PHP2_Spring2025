...
class AjaxController
{
    ...
    public function getUnit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['option'])) {
            $option = $_POST['option']; // Lấy toàn bộ mảng option
            $value = isset($option['value']) ? (int) $option['value'] : 0;
            $model = isset($option['model']) ?  $option['model'] : 0;
            $ware = new Material();
            $data = $ware->getOneMaterialById($value);
            if ($data && isset($data['unit'])) {
                echo json_encode(['status' => 'success', 'unit' => $data['unit']]);
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Không tìm thấy đơn vị.']);
            }
        }
    }
}
