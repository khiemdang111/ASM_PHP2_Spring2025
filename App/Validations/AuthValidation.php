...
class AuthValidation
{
  public static function register(): bool
  {
    ...
    if (!isset($_POST['email']) || trim($_POST['email']) === '') {
      ...
      if (!preg_match($emailPattern, $email)) {
        ...
        if ($db->connect_error) {
          NotificationHelper::error('database', 'Lỗi kết nối CSDL: ' . $db->connect_error);
          $is_valid = false;
        } else {
          ...
          if ($emailCount > 0) {
            NotificationHelper::error('email', 'Email đã tồn tại trong hệ thống');
            $is_valid = false;
          }
        }
  ...
  public static function login(): bool
  {
    ...
    if (!isset($_POST['email']) || $_POST['email'] === '') {
      NotificationHelper::error('email', 'Không để trống email đăng nhập');
      $is_valid = false;
    }
